...
@app.command()
def process(
    pdf: Optional[Path] = typer.Option(None, "--pdf", help="PDF file to process"),
    text: Optional[Path] = typer.Option(None, "--text", help="Text file to process"),
    ...
    with console.status("[bold green]Parsing file..."):
        if pdf:
            ...
            therapists = parser.parse_file(input_file)
        else:
...
def show_statistics(therapists, email_drafts) -> None:
